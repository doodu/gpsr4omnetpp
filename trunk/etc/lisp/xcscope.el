...
;;
;; File:         xcscope.el
;; RCS:          $RCSfile: xcscope.el,v $ $Revision: 1.1.1.1 $ $Date: 2006/01/06 01:43:59 $ $Author: chunywan $
...
(defun cscope-buffer-search (do-symbol do-next)
  ...
  (let* (line-number old-point point
                     ...
                     (buffer (cscope-get-output-buffer))
...
(defun cscope-display-buffer ()
  ...
  (interactive)
  (let ((buffer (cscope-get-output-buffer)))
    (if buffer
...
(defun cscope-pop-mark ()
  ...
  (let* ( (marker (ring-remove cscope-marker-ring 0))
          ...
          (cscope-buffer (cscope-get-output-buffer) ))
...
(defun cscope-bury-buffer ()
  ...
  (let ()
    ...
    (bury-buffer (cscope-get-output-buffer))
    ))
...
(defun cscope-quit ()
  (interactive)
  (cscope-bury-buffer)
  (let ((buf (cscope-get-output-buffer)))
...
(defun cscope-search-one-database ()
  ...
  (let ( next-item options cscope-directory database-file outbuf done
    ...
    (setq outbuf (cscope-get-output-buffer))
    (save-excursion
      (catch 'finished
        (set-buffer outbuf)
        (setq options '("-L"))
...
SENTINEL-FUNC are optional process filter and sentinel, respectively."
  (let ( (outbuf (cscope-get-output-buffer-create msg))
         (old-buffer (current-buffer)) )
    (if cscope-process
    ...
    (if (eq outbuf old-buffer) ;; In the *cscope* buffer.
        (if cscope-marker-window
            (progn
              ...
              (if (buffer-live-p cscope-marker-window)
                  (set-buffer cscope-marker-window)
                (set-buffer old-buffer))
      ...
      (progn ;; Not in the *cscope buffer.
        ...
        ;; was launched from.
        (setq cscope-marker-window old-buffer)
        (setq cscope-marker (point-marker))))
    (save-excursion
      (set-buffer outbuf)
      (if cscope-display-times
...
;;; wcy modified here
;;;
(defvar cscope-output-buffer-list nil
  "a list of buffer whose mode is cscope mode.\n")

(defun cscope-remove-killed-output-buffer()
    (setq cscope-output-buffer-list
        (remq nil (mapcar (lambda (buf) 
                            (and (buffer-live-p buf) buf))
                          cscope-output-buffer-list))))
(defun cscope-get-output-buffer-create(&optional name)
  ;; remove all killed buffer in the cscope-output-buffer-list first
  (cscope-remove-killed-output-buffer)
  (let ((buf (and name (get-buffer name))))
    (if (memq buf cscope-output-buffer-list)
        buf
      (setq cscope-output-buffer-list 
            (cons (get-buffer-create (or name "*my cscope*")) cscope-output-buffer-list))
      (car cscope-output-buffer-list))))

(defun cscope-get-output-buffer()
  (cscope-remove-killed-output-buffer)
  (car cscope-output-buffer-list))
